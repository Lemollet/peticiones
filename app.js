...
const peticionLibroTitulo = (titulo) => {
    request.get(url, (error, response, body) =>{
        response.statusCode === 200
        ? `Authors: ` + JSON.parse(body).docs.map(doc => doc.author_name.map(author_name =>console.log(author_name)))
        :
        console.log(error);  
    })
}
...
const peticionLibro = (autor) => {
    request.get(url, (error, response, boy) => {
        response.statusCode === 200
        ? console.log(`Sus libros son:` + JSON.parse(body).docs.map(doc => doc.tiitle_suggest))
        : console.log(error);
    })
}
